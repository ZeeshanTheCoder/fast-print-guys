...
const PrintBookCalculator = () => {
  ...
  const stepAccessibility = useMemo(() => ({
    ...
    paperType: form.bookSize !== '' && form.page_count !== '' && form.binding_id !== '' && form.interior_color_id !== '',
    coverFinish: form.bookSize !== '' && form.page_count !== '' && form.binding_id !== '' && form.interior_color_id !== '' && form.paper_type_id !== ''
  }), [form]);

  const bindingOptions = useMemo(() => ({
    paperback: Object.entries(BINDING_RULES)
      .filter(([, rules]) => rules.type === 'paperback')
      .map(([name, rules]) => ({ name, img: rules.img })),
  ...
  const isOptionAvailable = (optionType, optionValue) => {
    if (!form.bookSize) return true;
    const sizePricing = SIZE_SPECIFIC_PRICING[form.bookSize];
    const pricing = sizePricing?.[optionType];
    return pricing?.[optionValue] !== undefined && pricing[optionValue] >= 0;
  };
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!form.bookSize || !form.page_count || !form.binding_id || !form.interior_color_id || !form.paper_type_id || !form.cover_finish_id || form.quantity <= 0) {
    ...
    <>
      ...
      <div className="w-full min-h-screen px-4 sm:px-6 lg:px-8 py-8 flex flex-col xl:flex-row gap-6 xl:gap-8">
        <div className="w-full xl:w-3/5 bg-gradient-to-br from-blue-50 to-blue-100 p-4 sm:p-6 lg:p-8 rounded-2xl">
          <h2 className="text-2xl sm:text-3xl font-bold mb-6 text-[#2A428C]">Print Book</h2>
              ...
              <div className="w-full sm:w-1/2 flex flex-col">
                <div className="h-5 mb-1">
                  <p className="text-xs text-white opacity-90">
                    {form.bookSize ? 'MIN-MAX: 2 - 800' : 'Select book size first'}
                  </p>
                </div>
                <FormInput
                  name="page_count"
          ...
          <OptionSection
            ...
            options={bindingOptions.paperback}
          ...
          <OptionSection
            ...
            options={bindingOptions.hardcover}
            fieldName="binding_id"
            fieldValue={form.binding_id}
            onSelect={handleBindingSelect}
            isAvailable={(item) => availableBindings.includes(item.name)}
          ...
          <div className="flex gap-3 sm:gap-4 lg:gap-6 mb-8 sm:mb-10 flex-wrap">
            {OPTIONS_CONFIG.interiorColor.map((item, idx) => (
              <OptionCard
                ...
                isAvailable={isOptionAvailable('interiorColor', item.dbName)}
          ...
          <div className="flex gap-3 sm:gap-4 lg:gap-6 mb-8 sm:mb-10 flex-wrap">
            {OPTIONS_CONFIG.paperType.map((item, idx) => (
              <OptionCard
                ...
                isAvailable={isOptionAvailable('paperType', item.dbName)}
          ...
          <div className="flex gap-3 sm:gap-4 lg:gap-6 mb-8 sm:mb-10 flex-wrap">
            {OPTIONS_CONFIG.coverFinish.map((item, idx) => (
              <OptionCard
                ...
                isAvailable={isOptionAvailable('coverFinish', item.dbName)}
        ...
        <div className="w-full xl:w-2/5">
          <div className="sticky top-8 bg-white p-4 sm:p-6 lg:p-8 rounded-2xl shadow-lg">
            ...
            />
            <h2 className="text-lg sm:text-xl font-bold text-[#2A428C] mb-2 text-center">High-Quality Book Printing</h2>
            <div className="w-full h-0.5 bg-gray-300 mb-4"></div>

            {[
              [['Book Size', form.bookSize || '-'], ['Page Count', form.page_count || '-']],
