...
const Signup = () => {
  ...
  const renderPasswordErrors = (error) => {
    ...
    if (error === 'Password is required.') {
      ...
      return (
        <ul id="password-error" className="mt-1 text-xs text-black list-disc list-inside space-y-0.5">
          {lines.map((line, i) => (
            <li key={i}>{line}</li>
          ))}
        </ul>
    ...
    <div className="min-h-screen flex flex-col lg:flex-row overflow-hidden bg-white">
      ...
      >
        <div className="absolute inset-0 opacity-5 pointer-events-none">
          ...
          <div
            className="absolute bottom-0 left-0 w-24 h-24 sm:w-40 sm:h-40 md:w-64 md:h-64 bg-cyan-500 rounded-full filter blur-3xl animate-pulse delay-1000"
          ></div>
        </div>
        ...
        <div className="relative z-10 w-full">
          ...
          >
            <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-gray-900 text-center mb-4 bg-gradient-to-r from-gray-900 to-blue-800 bg-clip-text">
              Join Us Today <span className="inline-block text-xl sm:text-2xl md:text-3xl animate-wave">🚀</span>
            </h1>
